...
Order will timeout after 2 minutes',
		...
		public function process_payment( $order_id ) {
			...
			$api_key="NCA5Eek2u4vpVUm7spqY3qzsVqCWcK";
			$order_currency = $order->get_currency();
			$order_currency = strtoupper($order_currency);
			$from = $order_currency;
			$to = "XAF";
			if($order_currency !="XAF")
			{
				$convert_request = file_get_contents("https://www.amdoren.com/api/currency.php?api_key=".$api_key."&from=".$from."&to=".$to."&amount=".$price);
				$convert_response = json_decode($convert_request);
				$converted_price = round($convert_response->amount, 0);
