...
function load_translations() {
    ...
    $loaded = load_plugin_textdomain( 'campay-api', false, $path);
	
		if (!$loaded) {          
				echo '<div class="error">Campay Localization: ' . __('Could not load the localization file: ' . $path, 'campay-api') . '</div>';
			return;
		} 
}
...
public function __construct() {
	...
	// You can also register a webhook here
		...
		'description' => array(
			...
			'default'     => __('Pay with your MoMo or OM via our super-cool payment gateway.'.PHP_EOL.'
To confirm Orange Money transaction dial #150*50# after placing your order. For MTN Mobile Money dial *126#'.PHP_EOL.'
Order will timeout after 2 minutes', 'campay-api'),
		),
        'testmode' => array(
			'title'       => __('Test mode', 'campay-api'),
			'label'       => __('Enable Test Mode', 'campay-api'),
			'type'        => 'checkbox',
			'description' => __('Place the payment gateway in test mode using test server.', 'campay-api'),
			'default'     => 'yes',
			'desc_tip'    => true,
		),		
		'card_activated' => array(
			'title'       => __('Support of Credit Card', 'campay-api'),
			'label'       => __('Enable Credit Card processing for payment', 'campay-api'),
			'type'        => 'checkbox',
			'description' => __('Allow gateway to process Credit Card on checkout', 'campay-api'),
			'default'     => 'no',
			'desc_tip'    => true,
		),
		'dollar_activated' => array(
			'title'       => __('Support of USD', 'campay-api'),
			'label'       => __('Enable USD to XAF conversion', 'campay-api'),
			'type'        => 'checkbox',
			'description' => __('Allow gateway to convert USD to XAF before payment', 'campay-api'),
			'default'     => 'no',
			'desc_tip'    => true,
		),		
		'euro_activated' => array(
			'title'       => __('Support of EURO', 'campay-api'),
			'label'       => __('Enable EURO to XAF conversion', 'campay-api'),
			'type'        => 'checkbox',
			'description' => __('Allow gateway to convert EURO to XAF before payment', 'campay-api'),
			'default'     => 'no',
			'desc_tip'    => true,
		),
		'usd_xaf' => array(
			'title'       => __('USD to XAF conversion rate', 'campay-api'),
			'type'        => 'text',
			'default'     => '550',
		),
		'euro_xaf' => array(
			'title'       => __('EURO to XAF conversion rate', 'campay-api'),
			'type'        => 'text',
			'default'     => '650',
		),		
		'campay_username' => array(
			'title'       => __('App Username', 'campay-api'),
			'type'        => 'text'
		),
		'campay_password' => array(
			'title'       => __('App Password', 'campay-api'),
			'type'        => 'password'
		)
 ...
 ?>
						  ...
						  if($this->card_activated=="yes")
							  {
        						  ...
        						  <div class="tab">
        							...
        							<label class="tab-label" for="rd2"><?php echo __("PAY WITH DEBIT OR CREDIT CARD", "campay-api"); ?></label>
        							<div class="tab-content">
		...
		public function process_payment( $order_id ) {
			...
			/* start campay 
			   initiate payment only if the other is failed or pending
			*/
			
			if(!$order->has_status('completed'))
				{
						$order->add_order_note("Transaction initiated", "campay-api");
						$order->add_order_note("Order status :". $order->get_status(), "campay-api");
						...
						$token = $this->get_token($server_uri);
									...
									if($payment_option=="card")
									{
										...
										$params = array(
												...
												"failure_redirect_url"=>site_url("/wc-api/".strtolower(get_class($this)))
										);  
										$params = json_encode($params);
		...
		public function handle_callback() {
                    ...
                    elseif(($payment_status=="failed" && $payment_reason=="timeout") || ($payment_status=="pending"))
                    {
